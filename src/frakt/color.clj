...
(defn color [r g b] {:red (double r), :green (double g), :blue (double b)})

(def epsilon 0.00001)

(defn nearly-eq? [x y] (< (abs (- x y)) epsilon))
...
(defn color-add
  ...
  ([c1 c2]
   (color (+ (:red c1) (:red c2))
          (+ (:green c1) (:green c2))
          (+ (:blue c1) (:blue c2))))
  ([c1 c2 & more] (reduce color-add c1 (cons c2 more))))
...
(defn color-sub
  ...
  ([c1 c2]
   (color (- (:red c1) (:red c2))
          (- (:green c1) (:green c2))
          (- (:blue c1) (:blue c2))))
  ([c1 c2 & more] (reduce color-sub c1 (cons c2 more))))

(defn color-mul-scalar
  [c scalar]
  (color (* (:red c) scalar) (* (:green c) scalar) (* (:blue c) scalar)))
